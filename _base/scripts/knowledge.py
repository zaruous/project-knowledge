...
MANIFEST = Path("_base/manifest.yml")
PROJECT_CONFIG = Path("_config/project.yml")
...
def load_registry(root=ROOT):
    ...
    reg = Registry(
        types={k: dict(v, _template_root=str(TEMPLATE_DIR), _origin="base") for k, v in (types_doc.get("types") or {}).items()},
    ...
    if ext_path.exists():
        ...
        for name, spec in (ext.get("types") or {}).items():
            if name in reg.types:
                ...
                reg.types[name] = dict(spec, _template_root=".", _origin="project")
...
def module_dirs(manifest, modules):
    dirs = []
    for name in modules:
        dirs += (manifest.get("modules") or {}).get(name, {}).get("dirs", [])
    return dirs


def check_manifest(reg, manifest):
    """manifest의 모듈·프로필과 레지스트리 유형이 서로 맞는지 검사한다."""
    errors = []
    modules = manifest.get("modules") or {}
    for name, spec in reg.types.items():
        module = modules.get(spec.get("module"))
        if module is None:
            errors.append(f"types.{name}: manifest에 없는 모듈 '{spec.get('module')}'")
        elif spec["_origin"] == "base":
            base = spec["path"].rstrip("/")
            if not any(d == base or d.startswith(base + "/") for d in module.get("dirs", [])):
                errors.append(f"types.{name}: 저장 위치 {spec['path']}가 모듈 '{spec['module']}'의 폴더에 없다")
    for pname, profile in (manifest.get("profiles") or {}).items():
        for m in profile.get("modules", []):
            if m not in modules:
                errors.append(f"profiles.{pname}: manifest에 없는 모듈 '{m}'")
        dirs = [ph["dir"] for ph in profile.get("phases", [])]
        if len(dirs) != len(set(dirs)):
            errors.append(f"profiles.{pname}: 단계 폴더 이름이 중복된다")
    return errors
...
def validate(root=ROOT):
    ...
    if manifest:
        errors += check_manifest(reg, manifest)
    warnings = []
