...
class RegistryValidationTest(unittest.TestCase):
    ...
    # 추적 매트릭스 ----------------------------------------------------------
    def trace(self):
        return subprocess.run([sys.executable, str(ROOT / "_base/scripts/trace.py"), str(self.root)],
...
class RegistryDocsTest(unittest.TestCase):
    def test_naming_doc_matches_registry(self):
        text = (ROOT / "_base/framework/standards/naming.md").read_text(encoding="utf-8")
    ...
    @unittest.skipUnless(IS_TEMPLATE, "템플릿 저장소의 README에만 있는 표")
    def test_readme_profiles_match_manifest(self):
        text = (ROOT / "README.md").read_text(encoding="utf-8")
        for name, profile in knowledge.load_manifest(ROOT)["profiles"].items():
            row = f"| {name} | {', '.join(profile['modules'])} | {' · '.join(ph['dir'] for ph in profile['phases'])} |"
